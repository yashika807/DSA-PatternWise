...
slow = next(0) = (0+2)%5 = 2
fast = next(next(0)) -> next(0)=2, next(2)=3
slow(2) != fast(3) -> continue

slow = next(2) = 3
fast = next(next(3)) -> next(3)=0, next(0)=2... (recompute each round from current fast)
slow(3) == fast(3) -> length > 1 -> return true
